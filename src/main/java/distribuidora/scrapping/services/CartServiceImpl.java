...
public class CartServiceImpl implements CartService {
			...
			for (CartProductDto cp : cartDto.getProducts()) {
				...
				CategoryHasUnit currentCategory = categories.stream()
						.filter(c -> c.getCategory().getId()
								.equals(currentProductRelation.getProductoInterno().getLvCategoria().getId()))
	...
	public List<CartDto> getCarts() {
		...
		if (CollectionUtils.isNotEmpty(data)) {
			// Agrupo por cart
			TreeMap<Cart, List<CartProduct>> mapDataByCart = new TreeMap<Cart, List<CartProduct>>(
					(a, b) -> b.getDateCreated().compareTo(a.getDateCreated()));
			mapDataByCart.putAll(data.stream().collect(Collectors.groupingBy(d -> d.getCart())));
			// Itero cara key
