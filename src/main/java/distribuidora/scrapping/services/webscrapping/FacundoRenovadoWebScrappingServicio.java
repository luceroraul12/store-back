...
public class FacundoRenovadoWebScrappingServicio
	...
	@Override
	protected FacundoEntidad obtenerProductosAPartirDeElements(
			Element elementProducto) {
		String id = elementProducto.attr("data-idproducto");
		String nombre = elementProducto.getElementsByTag("h5").textNodes()
				.get(0).text().replaceAll("_",
						" ")
				.subSequence(0, elementProducto.getElementsByTag("h5")
						.textNodes().get(0).text().length() - 3)
				.toString();

		String categoria = elementProducto.parent().parent().parent().parent()
				.getElementsByTag("h2").text();

		Double precio = Double
				.valueOf(elementProducto.getElementsByTag("strong").text()
						.replaceAll(" ", "")
